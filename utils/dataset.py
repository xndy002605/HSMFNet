...
class FGRSC42Dataset(VisionDataset):

	def __init__(self, root, train=True, transform=None, target_transform=None):
		super(FGRSC42Dataset, self).__init__(root, transform=transform, target_transform=target_transform)
		self.loader = default_loader
		self.train = train
		self.samples = []
		...
		big_classes = sorted([d for d in os.listdir(self.data_dir) if os.path.isdir(os.path.join(self.data_dir, d)) and d != '.ipynb_checkpoints' ])

		cls_idx = 0

		for big_cls in big_classes:
			big_cls_path = os.path.join(self.data_dir, big_cls)

			small_classes = sorted(
				[d for d in os.listdir(big_cls_path) if os.path.isdir(os.path.join(big_cls_path, d)) and d != '.ipynb_checkpoints' ])

			for small_cls in small_classes:
				if small_cls not in self.class_to_idx:
					self.class_to_idx[small_cls] = cls_idx
					cls_idx += 1

				cls_path = os.path.join(big_cls_path, small_cls)

				for img_name in os.listdir(cls_path):
					img_path = os.path.join(cls_path, img_name)
					if not os.path.isfile(img_path):
						print(f"警告：文件 {img_path} 不是文件，已跳过")
						continue
					...
					try:
						...
						self.samples.append((img_path, small_cls))
					except PIL.UnidentifiedImageError:
						print(f"警告：文件 {img_path} 不是有效图像，已跳过")
					except Exception as e:
						print(f"处理 {img_path} 时出错：{e}，已跳过")



		self.idx_to_class = {v: k for k, v in self.class_to_idx.items()}
		self.class_names = [self.idx_to_class[i] for i in range(len(self.class_to_idx))]
		self.class_counts = defaultdict(int)
		for _, small_cls in self.samples:
			small_idx = self.class_to_idx[small_cls]
			self.class_counts[small_idx] += 1
		# 打印small_class_to_idx
		#print("class_to_idx:", self.class_to_idx)
		# # 新增：标记少数类（样本量<4的类别）
		# self.minority_classes = {
		# 	cls_idx for cls_idx, count in self.small_class_counts.items()
		# 	if count < 4
		# }
		# 在FGRSC42Dataset类的__init__方法末尾添加
		# if self.train:
		# 	# 仅在初始化训练集时检查与测试集的重叠
		...
		# 		# 收集测试集所有图像路径
		# 		test_samples = []
		# 		for big_cls in os.listdir(test_data_dir):
		# 			big_cls_path = os.path.join(test_data_dir, big_cls)
		# 			if not os.path.isdir(big_cls_path):
		# 				continue
		# 			for small_cls in os.listdir(big_cls_path):
		# 				small_cls_path = os.path.join(big_cls_path, small_cls)
		# 				if not os.path.isdir(small_cls_path):
		# 					continue
		# 				for img_name in os.listdir(small_cls_path):
		# 					test_samples.append(os.path.join(small_cls_path, img_name))
		# 		# 检查重叠
		# 		train_img_paths = [s[0] for s in self.samples]
		# 		overlap = set(train_img_paths) & set(test_samples)
		# 		if overlap:
		# 			raise ValueError(f"训练集与测试集存在{len(overlap)}个重复样本，如: {list(overlap)[:3]}")
	def __len__(self):
		return len(self.samples)

	def __getitem__(self, idx):
		img_path, small_cls = self.samples[idx]
		img = self.loader(img_path).convert('RGB')

		if not isinstance(img, Image.Image):
			img = Image.fromarray(img)
		small_target = self.class_to_idx[small_cls]

		if self.train:
			img = self.transform(img)
			# if small_idx in self.minority_classes and self.minority_transform is not None:
			...
			img = self.transform(img)
		if self.target_transform is not None:
			small_target = self.target_transform((small_target))

		label = torch.tensor(small_target, dtype=torch.long)
		return img, label
    
    
class FGSC23Dataset(VisionDataset):

	def __init__(self, root: str, train: bool = True, transform=None, target_transform=None):
		super().__init__(root, transform=transform, target_transform=target_transform)

		self.dataset_root = root
		self.split_dir = os.path.join(self.dataset_root, 'train' if train else 'test')

		if not os.path.exists(self.split_dir):
			raise FileNotFoundError(f"数据集目录不存在：{self.split_dir}\n请检查 root 路径和目录结构是否正确")
		...
		for folder_name in os.listdir(self.split_dir):
			...
			exclude_folders = ['.ipynb_checkpoints', '.DS_Store', '__pycache__']
			if folder_name in exclude_folders:
				print(f"提示：已跳过缓存/系统文件夹：{folder_path}")
				continue

			if not folder_name.isdigit():
				print(f"警告：文件夹 {folder_name} 名称非数字，不属于有效类别，已跳过（路径：{folder_path}）")
				continue
			...
			for entry_name in os.listdir(folder_path):
				...
				if entry_name in exclude_folders:
					print(f"提示：已跳过嵌套缓存文件夹：{entry_path}")
					continue

				if os.path.isfile(entry_path) and entry_name.lower().endswith(supported_extensions):
					self.img_paths.append(entry_path)
					self.labels.append(class_idx)

		if len(self.img_paths) == 0:
			raise RuntimeError(f"在 {self.split_dir} 中未找到任何有效图片文件，请检查数据集目录结构")
	...
	def get_class_distribution(self) -> dict:
		...
		return dict(sorted(class_dist.items(), key=lambda x: x[0]))

	@staticmethod
	def clean_ipynb_checkpoints(root: str):
		import shutil
		for root_dir, dirs, _ in os.walk(root):
			if '.ipynb_checkpoints' in dirs:
				checkpoint_dir = os.path.join(root_dir, '.ipynb_checkpoints')
				shutil.rmtree(checkpoint_dir)
				print(f"已删除缓存文件夹：{checkpoint_dir}")
		print("缓存文件夹清理完成！")
