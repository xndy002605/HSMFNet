...
def build_optimizer(config, model, backbone_low_lr=True):
	...
	if backbone_low_lr:
		parameters = set_weight_decay_backbone_lr(model, skip, skip_keywords)
	else:
...
def set_weight_decay_backbone_lr(model, skip_list=(), skip_keywords=()):
	has_decay_add = []
	has_decay_backbone = []
	no_decay_add = []
	no_decay_backbone = []
